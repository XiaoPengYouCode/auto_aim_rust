...
> 题外话：恭喜3SE战队成功从复活赛杀出国赛

## 8.6 凌晨

> 题外话：恭喜3SE战队成功进入 16 强
> 江油的事情令我很抑郁
> 影石面试加油！！！

1. 将 `serial` 和 `comm` 进行解耦，`serial` 模块提供串口读写功能，`comm` 模块提供通讯帧定义

## 8.18 晚上

看到大家的若值发言我感觉非常沉重，我决定继续打磨我的算法程序

专注于核心功能先，推迟暂停部分内容的开发

1. `hot_reload` 参数热更新
2. `estimator_poll` 估计器池
   - 先专注于实现单个目标的估计

为什么叫做 `estimator` 而不是 `tracker`，因为我们在做的任务是目标估计+跟踪，而非简单的目标跟踪
